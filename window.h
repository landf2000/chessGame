...
class Xwindow {
	...
	// if you use shared_from_this() function, make sure font exists on the undergrad environment
	...
	void fillPolygon(XPoint* points, int numPoints ,int shape,int colour=Black);
	// Draws a line from (x1, y1) to (x2, y2)
	void drawLine(int x1, int y1, int x2, int y2);

	void drawLines(XPoint *points, int numPoints, int colour);
